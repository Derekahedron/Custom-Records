...
public record StopSoundEffectPacket(
        Either<Tuple<ResourceKey<Level>, BlockPos>, Tuple<UUID, SlotReference>> location) {

    public StopSoundEffectPacket(ResourceKey<Level> dimension, BlockPos blockPos) {
        this(Either.left(new Tuple<>(dimension, blockPos)));
    }

    public StopSoundEffectPacket(UUID playerId, SlotReference slotReference) {
        this(Either.right(new Tuple<>(playerId, slotReference)));
    }

    public StopSoundEffectPacket(FriendlyByteBuf buffer) {
        this(
                buffer.readEither(
                        Tuple.reader(
